...
public interface SummaryProgramRewardGraphDAO {
	...
	public static String SELECT_BY_CHILD_TERRITORY_YTD = "SELECT [parent] 'parent', [Child] 'child' ,'' as toggle , ''  as 'program' ,SUM([Earnings]) 'earnings' FROM [SummaryProgramRewardGraph] where toggle = 'ytd' and child = ?0 group by parent, Child";
	public static String SELECT_BY_CHILD_TERRITORY_YTD_SINGLE = "SELECT [parent] 'parent', [Child] 'child' ,'' as toggle , ''  as 'program' ,SUM([Earnings]) 'earnings' FROM [SummaryProgramRewardGraph] where toggle = 'ytd' and child = ?0 group by parent, Child";
