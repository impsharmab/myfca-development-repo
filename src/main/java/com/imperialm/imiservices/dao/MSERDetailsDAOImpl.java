...
@Repository
public class MSERDetailsDAOImpl implements MSERDetailsDAO {
	...
	public List<MSERDetailsDTO> getMSERDetailsBySID(String territory) {
		List<MSERDetailsDTO> result = new ArrayList<MSERDetailsDTO>();

		try {
			final Query query = this.em.createNativeQuery(SELECT_BY_SID, MSERDetailsDTO.class);
			query.setParameter(0, territory);
			List<MSERDetailsDTO> rows = query.getResultList();
			result = rows;
