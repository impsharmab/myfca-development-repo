...
public interface RetentionGraphDAO {
	...
	public static String SELECT_NAT_BY_POSITIONCODE = "select '' as parentTerritory, 'NAT' childTerritory , positionCode, avg(percentage) 'percentage', '' as error from (select 'NAT' [Parent], [Parent] childTerritory , [PositionCode] positionCode, avg(Percentage) percentage from RetentionGraph where len(parentterritory) = 2 AND PositionCode = ?0 group by Parent , PositionCode) as t group by positionCode";
