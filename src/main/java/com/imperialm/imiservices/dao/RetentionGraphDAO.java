...
public interface RetentionGraphDAO {
	...
	public static String SELECT_BY_CHILD_TERRITORY_AND_POSITIONCODE = "SELECT [Parent] 'parentTerritory', [Child] 'childTerritory', [PositionCode] 'positionCode', [Percentage] 'percentage', '' as error FROM [dbo].[RetentionGraph] where [Child] like ?0 AND PositionCode = ?1";
	...
	//public static String SELECT_BY_BC_AND_POSITIONCODE = "select 'NAT' as parentTerritory, [Parent] childTerritory , [PositionCode] positionCode, avg(Percentage) percentage, '' as error from RetentionGraph where len(parent) = 2 AND PositionCode = ?0 and Parent = ?1 group by parent , PositionCode";
	public static String SELECT_BY_BC_AND_POSITIONCODE = "select 'NAT' as parentTerritory, [Parent] childTerritory , [PositionCode] positionCode, avg(Percentage) percentage, '' as error from RetentionGraph where len(parent) = 2 AND PositionCode = ?0 and Parent = ?1 group by parent , PositionCode";
	...
	public static String SELECT_NAT_BY_POSITIONCODE = "select 'NAT' as parent,'NAT' as child , PositionCode 'positionCode', ((CAST(sum(EndingPopulation) as DECIMAL)/(CAST(sum(BeginingPopulation) as DECIMAL)))*100) 'percentage', '' as error from dbo.RetentionGraph where parent = 'nat' and PositionCode = ?0 group by PositionCode";
	
	//public static String SELECT_NAT = "select  'NAT' as parentTerritory, [Parent] childTerritory , [PositionCode] positionCode, avg(Percentage) percentage, '' as error from RetentionGraph where len(parent) = 2  group by parent , PositionCode";
	public static String SELECT_NAT = "select 'NAT' as parent,'NAT' as child , PositionCode 'positionCode', ((CAST(sum(EndingPopulation) as DECIMAL)/(CAST(sum(BeginingPopulation) as DECIMAL)))*100) 'percentage', '' as error from dbo.RetentionGraph where parent = 'nat' group by PositionCode";
