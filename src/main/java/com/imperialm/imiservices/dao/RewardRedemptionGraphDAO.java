...
public interface RewardRedemptionGraphDAO {
	...
	public static String SELECT_BY_PARENT_TERRITORY_LIST = "SELECT [ParentTerritory] 'parentTerritory', [ChildTerritory] 'childTerritory', SUM([EarnedPoints]) 'earnedPoints', SUM([RedeemedPoints]) 'redeemedPoints', SUM([BalancePoints]) 'balancePoints', '' as 'program' FROM [dbo].[RewardRedemptionGraph] where [ParentTerritory] IN (?0) group by [ChildTerritory], [ParentTerritory]";
	public static String SELECT_BY_CHILD_TERRITORY_LIST = "SELECT [ParentTerritory] 'parentTerritory', [ChildTerritory] 'childTerritory', SUM([EarnedPoints]) 'earnedPoints', SUM([RedeemedPoints]) 'redeemedPoints', SUM([BalancePoints]) 'balancePoints', '' as 'program' FROM [dbo].[RewardRedemptionGraph] where [ChildTerritory] IN (?0) group by [ChildTerritory], [ParentTerritory]";
