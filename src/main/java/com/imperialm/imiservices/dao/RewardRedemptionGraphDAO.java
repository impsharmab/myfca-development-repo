...
public interface RewardRedemptionGraphDAO {
	...
	public static String SELECT_BY_CHILD_TERRITORY = "SELECT [Parent] 'parentTerritory', [Child] 'childTerritory', SUM([EarnedPoints]) 'earnedPoints', SUM([RedeemedPoints]) 'redeemedPoints', SUM([BalancePoints]) 'balancePoints', '' as 'program' FROM [dbo].[RewardRedemptionGraph] where [Child] like ?0 group by [Child], [Parent]";
