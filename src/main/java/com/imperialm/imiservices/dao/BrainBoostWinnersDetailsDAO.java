...
public interface BrainBoostWinnersDetailsDAO {
	...
	public static String SELECT_SUM_BY_SID = "SELECT '' as 'dealerCode' ,'' as 'dealerName' , '' as 'sID' ,'' as 'name' ,'' 'toggle' ,SUM([Points]) 'points' ,SUM([Partcipants]) 'partcipants' ,SUM([Winners]) 'winners' ,SUM([Earnings]) 'Earnings' FROM [BrainBoostWinnersDetails] where [SID] = ?0 AND Toggle = ?1";
