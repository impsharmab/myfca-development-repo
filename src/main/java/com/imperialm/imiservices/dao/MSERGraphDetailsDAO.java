...
public interface MSERGraphDetailsDAO {
	...
	public static String SELECT_DEALERCOUNT_BY_BC_OR_DISTRICT = "Select (Enrolled_Dealers) total, 'Total Dealers Enrolled' as 'name', '' as error From (Select MG.Parent, COUNT( DISTINCT Dealercode) Enrolled_Dealers From MSERGraphDetails MGD INNER JOIN MSERGraph MG ON MG.Child=MGD.DealerCode Where DealersEnrolled > 0 and MG.Parent like ?0 Group by MG.Parent) A";
	...
	public static String SELECT_TOTAL_EARNINGS_YTD_BY_BC_OR_DISTRICT = "Select IsNull(CAST(SUM([Amount]) as varchar(20)),'0') 'total', 'Excellence Card Awards YTD' as 'name', '' as error From (SELECT SUM([Amount]) Amount FROM [dbo].[MSERGraph] where child = ?0 and toggle = 'ytd' group by Child) A";
	public static String SELECT_TOTAL_EARNINGS_MTD_BY_BC_OR_DISTRICT = "Select IsNull(CAST(SUM([Amount]) as varchar(20)),'0') 'total', 'Excellence Card Awards MTD' as 'name', '' as error From (SELECT SUM([Amount]) Amount FROM [dbo].[MSERGraph] where child = ?0 and toggle = 'mtd' group by Child) A";
